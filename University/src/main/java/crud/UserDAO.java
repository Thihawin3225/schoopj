...
public class UserDAO {
    ...
    public List<User> searchUsers(String searchQuery) {
        ...
        try (Connection connection = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD);
             PreparedStatement preparedStatement = connection.prepareStatement(
                     "SELECT * FROM user WHERE REPLACE(uniname, ' ', '') LIKE ? OR REPLACE(location, ' ', '') LIKE ? OR mark < ? OR REPLACE(link, ' ', '') LIKE ?")) {
    ...
    public List<User> searchAnd(String uninameQuery, String locationQuery, String markQuery) {
        ...
        try (Connection connection = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD);
             PreparedStatement preparedStatement = connection.prepareStatement(
                     "SELECT * FROM user WHERE REPLACE(uniname, ' ', '') LIKE ? AND REPLACE(location, ' ', '') LIKE ? AND mark < ?")) {

            // Remove spaces from uninameQuery, locationQuery, and markQuery
            String uninameWithoutSpaces = uninameQuery.replaceAll("\\s+", "");
            String locationWithoutSpaces = locationQuery.replaceAll("\\s+", "");

            preparedStatement.setString(1, "%" + uninameWithoutSpaces + "%");
            preparedStatement.setString(2, "%" + locationWithoutSpaces + "%");
    ...
    public List<User> searchUniNameAndLocation(String uninameQuery, String locationQuery) {
        ...
        try (Connection connection = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD);
             PreparedStatement preparedStatement = connection.prepareStatement(
                     "SELECT * FROM user WHERE REPLACE(uniname, ' ', '') LIKE ? AND REPLACE(location, ' ', '') LIKE ?")) {

            // Remove spaces from uninameQuery and locationQuery
            String uninameWithoutSpaces = uninameQuery.replaceAll("\\s+", "");
            String locationWithoutSpaces = locationQuery.replaceAll("\\s+", "");
    ...
    public List<User> searchLocationAndMark(String locationQuery, String uniMarkQuery) {
        ...
        try (Connection connection = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD);
             PreparedStatement preparedStatement = connection.prepareStatement(
                     "SELECT * FROM user WHERE REPLACE(location, ' ', '') LIKE ? AND mark < ?")) {

            // Remove spaces from the locationQuery
            String locationWithoutSpaces = locationQuery.replaceAll("\\s+", "");

            preparedStatement.setString(1, "%" + locationWithoutSpaces + "%");
    ...
    public List<User> searchUniNameAndMark(String uninameQuery, String uniMark) {
        ...
        try (Connection connection = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD);
             PreparedStatement preparedStatement = connection.prepareStatement(
                     "SELECT * FROM user WHERE REPLACE(uniname, ' ', '') LIKE ? AND mark < ?")) {

            // Remove spaces from the uninameQuery
            String uninameWithoutSpaces = uninameQuery.replaceAll("\\s+", "");
...
public List<User> searchLocation(String locationQuery) {
    ...
    try (Connection connection = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD);
         PreparedStatement preparedStatement = connection.prepareStatement(
                 "SELECT * FROM user WHERE REPLACE(location, ' ', '') LIKE ?")) {

        // Remove spaces from the locationQuery
        String locationWithoutSpaces = locationQuery.replaceAll("\\s+", "");

        // Use '%locationWithoutSpaces%' to perform a partial match
        preparedStatement.setString(1, "%" + locationWithoutSpaces + "%");
...
public List<User> searchUni(String uniQuery) {
    ...
    try (Connection connection = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD);
         PreparedStatement preparedStatement = connection.prepareStatement(
                 "SELECT * FROM user WHERE REPLACE(uniname, ' ', '') LIKE ?")) {

        // Remove spaces from the uniQuery
        String uniWithoutSpaces = uniQuery.replaceAll("\\s+", "");

        // Use '%uniWithoutSpaces%' to perform a partial match
        preparedStatement.setString(1, "%" + uniWithoutSpaces + "%");
